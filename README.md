...
* Access logs share Typecho's own database by default, but the plugin settings can point them at a separate MySQL / PostgreSQL server instead (host, port, user, password, database, table prefix). The target database must already exist — the plugin only creates the table. The connection is tested before the settings are saved, and article titles are resolved with a second query against Typecho's own database since a cross-database JOIN is not possible. Existing stats are migrated to the new database: up to 50k rows are moved inline when the settings are saved; beyond that the admin notice points at `php usr/plugins/Access/tools/migrate.php`, a CLI migrator that uses keyset pagination and batched inserts, is not subject to web server timeouts, and resumes from where it left off if interrupted. An incomplete migration is always reported as incomplete — it is never silently treated as done.

...
* 统计数据默认与 Typecho 共用一个库，也可以在插件设置里单独指定一个 MySQL / PostgreSQL 连接（地址、端口、用户名、密码、库名、表前缀），把访问日志与博客数据分开存放：
  ...
  * 切换到独立数据库时会把主库中已有的统计数据迁移过去：不超过 5 万行在后台直接完成，超过则提示改用命令行脚本 `php usr/plugins/Access/tools/migrate.php`（keyset 分页 + 批量写入，支持断点续传，不受 Web 超时限制）；
  * 迁移未完成时后台会明确显示剩余行数，不会把截断当成成功；重新保存设置或重跑脚本都会从断点继续；
